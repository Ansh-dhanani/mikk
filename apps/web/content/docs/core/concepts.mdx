...
Mikk uses **web-tree-sitter** and the **TypeScript Compiler API** to extract real ASTs (not regex sweeps) across 13 programming languages:

| Node type | Captured fields |
|-----------|----------------|
| Typescript / JS | Names, params + types, return type, start/end line, internal calls, generics |
| Python, Java, C++, C#, Rust, PHP, Ruby | AST extraction via Tree-Sitter for methods, classes, and cross-file imports |
| Go | High-fidelity extraction via Tree-Sitter + Go specific AST matching |
