---
title: CLI Reference
description: All 17+ commands for the Mikk CLI — with options, flags, and examples.
---
...
List all architectural decisions tracked by Mikk.
...
Get the full details and reasoning for a specific decision.
...
### `mikk adr add` / `mikk adr rm`

Add a new decision or remove an existing one. Options include `--id`, `--title`, and `--reason`.

```bash
mikk adr add --id "hybrid-search" --title "Use BM25" --reason "We need keyword support"
mikk adr rm hybrid-search
```
...
## Refactoring & Version Control commands

### `mikk rename`

Execute a multi-file coordinated rename. Finds the definition, updates the original export, and updates every call-site/import-site.
...
Trace Git diff hunks to actual affected functions and modules. See what code changes actually mean for execution flow.
...
<TypeTable
  type={{
    ...
    "mikk remove": {
      description: "Completely uninstall Mikk and physically delete the target project directories and the mikk.json contract.",
      type: "command",
    },
    "mikk version": {
