...
export function registerRefactorTools(server: McpServer, projectRoot: string) {
    ...
    server.tool(
        ...
        async ({ functionName, newName }: any) => {
            ...
            const stringReferences: Array<{ file: string; line: number; text: string }> = []
            const strRegex = new RegExp(`['"\`].*?\\b${functionName}\\b.*?['"\`]`)

            await Promise.all(Object.keys(lock.files).map(async (relPath) => {
                try {
                    const absPath = path.isAbsolute(relPath) ? relPath : path.join(projectRoot, relPath)
                    const content = await fs.readFile(absPath, 'utf-8')
                    if (!content.includes(functionName)) return
                    const lines = content.split('\n')
                    for (let i = 0; i < lines.length; i++) {
                        if (strRegex.test(lines[i])) {
                            stringReferences.push({ file: relPath, line: i + 1, text: lines[i].trim() })
                        }
                    }
                } catch (e) { /* ignore */ }
            }))

            const warnings = stringReferences.map(ref => `Potential string-based reference to "${functionName}" in ${ref.file}:${ref.line}`)
            ...
            return {
                content: [{
                    type: 'text' as const, text: JSON.stringify({
                        ...
                        summary: { callSites: callers.length, importSites: filesImporting.length, stringReferences: stringReferences.length, totalEdits: editPlan.length },
                        editPlan,
                        callers,
                        filesImporting,
                        stringReferences,
                        warnings: warnings.length > 0 ? warnings : undefined,
                        warning: staleness || undefined,
                        hint: `Execute all ${editPlan.length} edits, review ${stringReferences.length} string references, then run \`mikk analyze\`.`,
                    }, null, 2)
                }]
            }
        },
