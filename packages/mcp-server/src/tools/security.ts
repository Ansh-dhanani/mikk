...
import { z } from 'zod'
import { loadContractAndLock, isSourceFile, requestQueue, PRIORITIES, calculateEntropy } from './shared.js'
...
const SECRET_SCAN_PATTERNS = [
    ...
    { id: 'openai_key', pattern: /sk-(?:proj-)?[A-Za-z0-9_-]{32,}/, severity: 'critical', label: 'OpenAI API Key', envVar: 'OPENAI_API_KEY' },
    ...
    { id: 'generic_api_key', pattern: /(?:api[_-]?key|apikey)\s*[=:]\s*["']([A-Za-z0-9_-]{16,})["']/i, severity: 'high', label: 'Hardcoded API Key', envVar: 'API_KEY' },
    ...
    // Regex for secrets in multi-line template literals (dotall)
    { id: 'template_secret', pattern: /`[\s\S]*?(?:api[_-]?key|secret|password|passwd|pwd|openai|sk-)[\s\S]*?`/, severity: 'high', label: 'Potential Secret in Template Literal', envVar: 'TEMPLATE_SECRET' },
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // OBFUSCATION & EVASION (T48)
    // ═══════════════════════════════════════════════════════════════════════
    { id: 'obfuscated_secret', pattern: /(?:btoa|Buffer\.from|atob)\(['"](?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?['"]\)/, severity: 'high', label: 'Base64 Obfuscated Secret', envVar: '' },
    { id: 'dynamic_eval', pattern: /new\s+Function\(|eval\s*\(\s*atob\(/, severity: 'critical', label: 'Dynamic Exec (Obfuscated)', envVar: '' },
    // T48 Level 2 fix: detect partial OpenAI/Anthropic key fragments in quoted strings
    // (split-variable obfuscation — key split across P1+P2+P3)
    { id: 'openai_key_fragment', pattern: /["'`]sk-(?:proj-|ant-)[A-Za-z0-9_-]{4,}["'`]/, severity: 'high', label: 'OpenAI Key Fragment (split obfuscation)', envVar: 'OPENAI_API_KEY' },
    // T48 Level 3 fix: detect hex-escape obfuscated strings (repeated \xNN sequences)
    // Common pattern: const K = `\x73\x6b\x2d...` to hide sk-proj-... from scanners
    ...
    ; (server as any).tool(
        ...
        async (args: any): Promise<any> => {
            return requestQueue.add(async () => {
                const { path: scanPath, recursive, severity, exclude, projectRoot: argRoot } = args as any
                ...
                if (scanPath) {
                    const baseDir = path.isAbsolute(scanPath) ? scanPath : path.join(effectiveRoot, scanPath)
                    const walk = async (dir: string, depth = 0): Promise<void> => {
                        ...
                        try {
                            for (const entry of await fs.readdir(dir, { withFileTypes: true })) {
                                ...
                                // T48/T56 fix: use effectiveRoot (not module-level projectRoot) for relative path calc
                                else if (entry.isFile() && isSourceFile(full) && !isExcluded(path.relative(effectiveRoot, full), excludePatterns)) filesToScan.push(path.relative(effectiveRoot, full))
                            }
                        } catch { /* skip */ }
                    }
                    await walk(baseDir)
                } else if (Object.keys(lock.files).length > 0) {
                    // lock has tracked files — scan them
                    // lock.files keys may be absolute paths (normalized lowercase, forward slashes)
                    // or relative paths. Normalise to relative for consistent exclusion checks.
                    for (const absOrRel of Object.keys(lock.files)) {
                        const full = path.isAbsolute(absOrRel)
                            ? absOrRel
                            : path.join(effectiveRoot, absOrRel)
                        const rel = path.relative(effectiveRoot, full).replace(/\\/g, '/')
                        if (isSourceFile(full) && !isExcluded(rel, excludePatterns)) filesToScan.push(rel)
                    }
                } else {
                    // T48/T56 fix: no scanPath and no lock entries (fresh temp dir after index_project may
                    // still have sparse lock). Fall back to walking effectiveRoot directly.
                    const walk = async (dir: string, depth = 0): Promise<void> => {
                        if (depth > 10 || isExcluded(path.relative(effectiveRoot, dir), excludePatterns)) return
                        try {
                            for (const entry of await fs.readdir(dir, { withFileTypes: true })) {
                                const full = path.join(dir, entry.name)
                                const rel = path.relative(effectiveRoot, full)
                                if (entry.isDirectory()) { if (recursive !== false && !isExcluded(rel, excludePatterns)) await walk(full, depth + 1) }
                                else if (entry.isFile() && isSourceFile(full) && !isExcluded(rel, excludePatterns)) filesToScan.push(rel)
                            }
                        } catch { /* skip */ }
                    }
                ...
                for (const relFile of filesToScan) {
                    // relFile may be relative or absolute; always resolve to absolute before reading
                    const fullPath = path.isAbsolute(relFile) ? relFile : path.join(effectiveRoot, relFile)
                    // Use a normalised relative path for findings so callers can match by filename
                    const displayFile = path.relative(effectiveRoot, fullPath).replace(/\\/g, '/')
                    let content: string
                    try { content = await fs.readFile(fullPath, 'utf-8') } catch { continue }

                    // Check for multi-line secrets (rolling window or full content check)
                    for (const pat of SECRET_SCAN_PATTERNS) {
                        if ((SEVERITY_ORDER[pat.severity] ?? 0) < minSev) continue
                        const flags = (pat.pattern.flags || '') + 'g' + ((pat.pattern.flags || '').includes('m') ? '' : 'm') + (pat.id === 'template_secret' ? 's' : '')
                        const globalPat = new RegExp(pat.pattern.source, flags)
                        let match: RegExpExecArray | null
                        while ((match = globalPat.exec(content)) !== null) {
                            // Extract value from first capturing group or the whole match
                            const val = match[1] || match[0]
                            if (!val || val.includes('${') || val.includes('process.env')) continue

                            // For generic patterns, check for entropy to reduce false positives
                            if (pat.id === 'generic_secret' || pat.id === 'generic_api_key') {
                                if (calculateEntropy(val) < 3.0) continue
                            }
                            ...
                            if (pat.needsContext && !pat.needsContext.some((c: string) => lineContent.toLowerCase().includes(c))) continue

                            findings.push({
                                file: displayFile, line: lineNo, severity: pat.severity, type: pat.label, id: pat.id,
                                envVar: pat.envVar || null, context: lineContent.trim().slice(0, 100),
                                valuePreview: val.slice(0, 8) + '...'
                            })
                        }
                    }

                    // T48/T56 fix: Support multi-line templates and backtick strings
                    // ─── High Entropy Detection (AST-lite) ───────────────────────────
                    // Scan for high-entropy strings (quotes, backticks, multiline templates)
                    // Also detect secrets in template literals (backtick strings)
                    const entropyRegex = /"(?:[^\x00-\x1f\\]|\\.){20,}"|'(?:[^\x00-\x1c\\]|\\.){20,}'|`(?:[^`\\]|\\.){20,}`/g
                    let eMatch
                    while ((eMatch = entropyRegex.exec(content)) !== null) {
                        // Strip quotes/backticks
                        const val = eMatch[0].slice(1, -1)
                        // Filter out common noise: very long strings, potential interpolation, or non-secrets
            ...
            async (args: any): Promise<any> => {
                return requestQueue.add(async () => {
                    ...
                    const REPLACE_PATTERNS = [
                        ...
                        { id: 'openai-key', regex: /(sk-(?:proj-)?[A-Za-z0-9_-]{32,})/, valueGroup: 1, fixedEnvName: 'OPENAI_API_KEY' },
